...
- **Screen recording** (`start_recording`, `stop_recording`): ffmpeg `gdigrab` capture. Only failed-run recordings are kept.
...
- **Sleep prevention** (`prevent_sleep`, `allow_sleep`): Windows `SetThreadExecutionState` API.

## Key Design Details

- **Windows-only**: Uses `msvcrt` (ESC key listener), `ctypes.windll` (sleep prevention), `gdigrab` (screen capture), `taskkill` (process management).
